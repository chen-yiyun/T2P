...
class T2PDataset(Dataset):
    ...
    def get(self, idx):
        ...
        if self.transform:
            ...
            data["body_xyz"] = torch.tensor(
                np.asarray(pcd_EulerAngle.points).reshape(-1, 75, 45)
            )
        ...
        # 重塑数据维度以匹配每个场景的人数
        data["input_seq"] = input_seq.reshape(
            input_seq.shape[0] // self.num_person,
            self.num_person,
            input_seq.shape[1],
            -1,
        )
        data["output_seq"] = output_seq.reshape(
            output_seq.shape[0] // self.num_person,
            self.num_person,
            output_seq.shape[1],
            -1,
        )
